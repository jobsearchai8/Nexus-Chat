 ...
 * Uses Google Identity Services (GIS) popup for Google sign-in
 * Shows "nexus-networks.vercel.app" in Google prompt (not supabase.co)
 */
...
import { Loader2, Zap, ArrowRight, Shield, Globe, MessageCircle } from "lucide-react";
...
export default function Login() {
  ...
  const { signIn, signInWithGoogle, renderGoogleButton, isDemo, user, isLoading: authLoading } = useAuth();
  ...
  // Render Google button using GIS
  useEffect(() => {
    if (googleBtnRendered.current || isDemo || !googleBtnRef.current) return;
  ...
  }, [isDemo, renderGoogleButton, authLoading]);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // For non-demo, the GIS callback will trigger signInWithIdToken
      // and onAuthStateChange will set the user
    } catch (error: any) {
    ...
    <div className="min-h-screen flex bg-[#F0F2F5]">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12">
          ...
          >
            <h1 className="text-4xl font-extrabold text-white leading-tight mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Connect, Share,
              <br />
              <span className="text-[#4ECDC4]">Communicate</span>
            </h1>
            <p className="text-white/70 text-lg leading-relaxed">
              Your all-in-one platform for social networking and real-time messaging. Share thoughts, connect with friends, and chat — all in one place.
            </p>
          </motion.div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        ...
        >
          ...
          {/* Google Sign In — GIS rendered button (shows app domain, not supabase.co) */}
          {!isDemo && (
            <div className="mb-4">
              {/* GIS will render its button here */}
              <div ref={googleBtnRef} className="w-full flex justify-center" />
              {/* Fallback button if GIS doesn't render */}
              <Button
                variant="outline"
                className="w-full h-11 mt-2 bg-white border-gray-200 hover:bg-gray-50 text-[#1C1E21] transition-colors shadow-sm"
          ...
          {/* Demo mode — single Google button */}
          {isDemo && (
            <Button
              variant="outline"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-sm font-medium text-[#1C1E21]">
                Email
              </Label>
              <Input
                id="email"
          ...
          {isDemo && (
            <Button
              variant="ghost"
              className="w-full mt-3 text-[#1877F2] hover:text-[#166FE5] hover:bg-[#E7F3FF]"
